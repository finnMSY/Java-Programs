...
public class A3  extends JFrame {
	...
	class AddActionListener implements ActionListener {
		public void actionPerformed(ActionEvent e) {
			AnimationViewer a = new AnimationViewer();
			NestedShape s = new NestedShape();
			try {
				s = (NestedShape)tree.getLastSelectedPathComponent();
				panel.addShapeNode(s);
			}
			catch(Exception exp) {
				if (s == null) {
	...
	class RemoveActionListener implements ActionListener{
		public void actionPerformed(ActionEvent e){
			...
			else if (!panel.isRoot((Shape) node)){
